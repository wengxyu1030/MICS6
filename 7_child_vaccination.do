...
pause vac
/*
sum im6my // or alternative for measles

sum im26 im2 im5 im11
...
*/
	...
	*c_measles: Child age 15-23 immunized against measles/MMR
		...
		if inlist(country_name,"KyrgyzRepublic2018","Mongolia2018","NorthMacedonia2018","Uzbekistan2021") {
       ...
       //  need to combine with FORM FOR VACCINATION RECORDS AT HEALTH FACILITY	   
		...
		/*
		if inlist(country_name,"SaoTomeAndPrincipe2019") {
			replace c_measles = 1 if c_measles == 0 & (inrange(im6my,2000,6666) | inlist(im6md,44,66) | inrange(im6m2y,2000,6666) | inlist(im6m2d,44,66))			// measles/MMR from card
			replace c_measles = 1 if c_measles == 0 & im26 == 1				// measles/MMR from memory
			replace c_measles = . if ((inrange(im5,1,3) & (inrange(im6my,6667,9999) | inrange(im6md,97,98) | inrange(im6m2y,6667,9999) | inlist(im6m2y,0,97,98))) | (im11 == 1 & inlist(im26,8,9)) | (inlist(im2,8,9) & inlist(im11,8,9)))	// missing if measles DK/missing for card and memory
		}		
		*/
		if inlist(country_name,"Fiji2021") {
			...
			replace c_measles = 1 if c_measles == 0 & im26 == 1				// measles/MMR from memory
			replace c_measles = . if ((inrange(im5,1,3) & (inrange(im6m1y,6667,9999) | inrange(im6m1d,97,98) | inrange(hf12m1y,6667,9999) | inlist(hf12m1d,0,97,98))) | (im11 == 1 & inlist(im26,8,9)) | (inlist(im2,8,9) & inlist(im11,8,9)))	// missing if measles DK/missing for card and memory
			replace c_measles = . if ((inrange(im5,1,3) & (inrange(im6m2y,6667,9999) | inrange(im6m2d,97,98) | inrange(hf12m2y,6667,9999) | inlist(hf12m2d,0,97,98))) | (im11 == 1 & inlist(im26,8,9)) | (inlist(im2,8,9) & inlist(im11,8,9)))	// missing if measles DK/missing for card and memory
		}
		if inlist(country_name,"Nigeria2021") {
			replace c_measles = 1 if c_measles == 0 & (inrange(im6n1y,2000,6666) | inlist(im6n1d,44,66))
			replace c_measles = 1 if c_measles == 0 & (inrange(im6n2y,2000,6666) | inlist(im6n2d,44,66))			// measles/MMR from card
			replace c_measles = 1 if c_measles == 0 & im26 == 1				// measles/MMR from memory
			replace c_measles = . if ((inrange(im5,1,3) & (inrange(im6n1y,6667,9999) | inrange(im6n1d,97,98))) | (im11 == 1 & inlist(im26,8,9)) | (inlist(im2,8,9) & inlist(im11,8,9)))	// missing if measles DK/missing for card and memory
			replace c_measles = . if ((inrange(im5,1,3) & (inrange(im6n2y,6667,9999) | inrange(im6n2d,97,98))) | (im11 == 1 & inlist(im26,8,9)) | (inlist(im2,8,9) & inlist(im11,8,9)))	// missing if measles DK/missing for card and memory
		}
    ...
    * c_bcg: Child age 15-23M had BCG vaccination
		...
		if inlist(country_name,"KyrgyzRepublic2018","Mongolia2018","Congodr2017","Fiji2021","Uzbekistan2021") {
	...
	* c_dpt3: Child age 15-23M had DPT3 or Pentavalent 3 vaccination
	...
	forvalues x = 1 2 to 3 {
	    ...
	    #delimit cr		
			...
			if inlist(country_name,"KyrgyzRepublic2018","Mongolia2018","Congodr2017","Tonga2019","Fiji2021","Uzbekistan2021"){
	...
	* c_polio3: Child age 15-23M received polio3/OPV3 vaccination
    ...
    if inlist(country_name,"Suriname2018", "CostaRica2018","Tonga2019") {
		...
		forvalues x = 1 2 to 3 {
		   ...
		   if inlist(country_name,"SierraLeone2017","Iraq2017","Gambia2018","Lesotho2018","Madagascar2018","Ghana2017","Togo2017","Kiribati2018","Thailand2019")|inlist(country_name,"StateofPalestine2019","Nepal2019","Algeria2018","CentralAfricanRepublic2018","SaoTomeAndPrincipe2019","Samoa2019","Nigeria2021","Guyana2019")  {
		       replace c_polio`x' = 1 if c_polio`x' == 0 & (inrange(im6p`x'y,2000,6666) | inlist(im6p`x'd,44,66))
		       replace c_polio`x' = 1 if c_polio`x' == 0 & im16 == 1 & ((im17 == 2 & inrange(im18,`x',7)) | inrange(im18,`x'+1,7))   // polio1-3 from memory
			   replace c_polio`x' = . if ((inrange(im5,1,3) & (inrange(im6p`x'y,6667,9999) | inrange(im6p`x'd,97,98))) | (im11 == 1 & (inlist(im16,8,9) | inlist(im17,8,9) | inlist(im18,8,9))) | (inlist(im2,8,9) & inlist(im11,8,9))) // missing if Polio1-3 DK/missing for card and memory
		   }
